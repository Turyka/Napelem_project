...
def login():
    ...
    token, error = API_login.login_to_laravel_api(username, password)

    if token:
        messagebox.showinfo("Helyes Login", f"Login successful!\nYour token: {token}")
    else:
        messagebox.showerror("Helytelen login", error)
...
# Login Button
btn_login = tk.Button(frame, text="Login", font=("Arial", 12, "bold"), bg=BUTTON_COLOR, fg="white",
                      padx=10, pady=5, width=15, borderwidth=0, relief="flat", command=login)
